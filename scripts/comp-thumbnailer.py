#!/usr/bin/env python3
"""
Thumbnailer for .comp files (Company Editor).

Freedesktop thumbnailer spec:
    comp-thumbnailer -s SIZE URI OUTPUT_PATH

Usage:
    comp-thumbnailer -s 256 file:///path/to/file.comp /tmp/thumb.png
"""
...
def generate_thumbnail(size: int, output_path: str):
    """Generate a placeholder thumbnail for .comp files."""
    ...
    # Company blue gradient background
    draw.rounded_rectangle(
        [(margin, margin), (size - margin, size - margin)],
        radius=r,
        fill=(255, 152, 0, 255),
        outline=(230, 130, 0, 255),
        width=max(2, size // 64),
    )
